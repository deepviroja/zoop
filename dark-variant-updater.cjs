...
function processFile(filePath) {
  ...
  for (let [oldClass, newClass] of replacements) {
    ...
    content = content.replace(regex, `${oldClass} dark:${newClass}`);
  }

  // Handle aspect ratio universally (not dark variant)
  const aspectRegex = new RegExp(`(?<=[\\'\\"\\s\`])aspect-square(?=[\\'\\"\\s\`])`, 'g');
  content = content.replace(aspectRegex, 'aspect-4-5');
