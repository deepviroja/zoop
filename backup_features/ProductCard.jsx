...
const ProductCard = ({ product, view = "grid" }) => {
  ...
  if (view === "list") {
    return (
      <Link
        ...
        className="block glass-card border border-white/10 rounded-xl overflow-hidden hover:shadow-2xl transition-all group"
      >
        <div className="flex gap-6 p-6">
          ...
          <div className="w-48 h-48 bg-zoop-canvas rounded-lg overflow-hidden shrink-0 relative">
            ...
            />
            {product.type === "Local" && (
              <div className="absolute top-2 left-2 bg-zoop-moss text-white text-[10px] font-black px-2 py-1 rounded-full shadow-[0_12px_32px_rgba(0,0,0,0.5)] flex items-center gap-1">
            ...
            <div>
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                  ...
                  <h3 className="text-xl font-bold text-white mt-1 line-clamp-2 group-hover:text-zoop-moss transition-colors">
  ...
  return (
    <>
      <Link
        to={`/product/${product.id}`}
        className="block glass-card border border-white/10 rounded-2xl overflow-hidden hover:shadow-2xl transition-all group"
      ...
      >
        ...
        <div className="relative aspect-4-5 bg-zoop-canvas overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {discount > 0 && (
              <div className="bg-red-500 text-white text-xs font-black px-3 py-1 rounded-full shadow-[0_12px_32px_rgba(0,0,0,0.5)]">
                {discount}% OFF
              </div>
            )}
            {product.type === "Local" && (
              <div className="bg-zoop-moss text-white text-[10px] font-black px-3 py-1 rounded-full shadow-[0_12px_32px_rgba(0,0,0,0.5)] flex items-center gap-1">
                <Zap width={10} height={10} fill="black" /> SAME-DAY
              </div>
            )}
            {/* Show tag only if no discount and not local delivery */}
            {!discount && product.type !== "Local" && product.tag && (
              <div className="bg-white/90 backdrop-blur text-white text-[10px] font-black px-3 py-1 rounded-full">
                {product.tag}
              </div>
            )}
          </div>
          ...
          <div className="absolute top-3 right-3 z-10">
            <button
              onClick={handleLike}
              className="p-2 bg-white/90 backdrop-blur rounded-full hover:bg-white transition-all shadow-[0_12px_32px_rgba(0,0,0,0.5)] hover:scale-110 active:scale-95 cursor-pointer"
        ...
        <div className="p-4">
          ...
          {/* Product Name */}
          <h3 className="text-base font-bold text-white line-clamp-2 min-h-[3rem] group-hover:text-zoop-moss transition-colors">
            {product.title || product.name}
          </h3>
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <button
              ...
              className="md:hidden bg-zoop-obsidian text-white p-2.5 rounded-xl hover:bg-zoop-moss hover:text-zoop-obsidian transition-colors shadow-[0_12px_32px_rgba(0,0,0,0.5)] active:scale-95"
            >
              <ShoppingCart width={18} height={18} />
            </button>
          </div>
      ...
      {isQuickAddOpen && (
        ...
        >
          ...
          <div
            className="w-full glass-card rounded-t-[2rem] p-6 relative z-10 animate-in slide-in-from-bottom duration-300"
          ...
          >
              ...
              {sizes.map((size) => (
                <button
                  ...
                  className={`w-12 h-12 rounded-xl flex items-center justify-center font-black transition-all ${
                    selectedSize === size
                      ? "bg-zoop-obsidian text-white shadow-[0_12px_32px_rgba(0,0,0,0.5)] scale-110"
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 ${
                selectedSize
                  ? "bg-zoop-moss text-white shadow-[0_12px_32px_rgba(0,0,0,0.5)] shadow-zoop-moss/20"
