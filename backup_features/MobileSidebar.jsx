...
const MobileSidebar = ({ isOpen, onClose, categories = [], quickLinks = [] }) => {
  ...
  return (
    <>
      ...
      {/* --- SIDEBAR PANEL --- */}
      <aside
        className={`fixed top-0 left-0 h-screen w-[320px] max-w-[88vw] glass-card z-[201] shadow-[0_24px_64px_rgba(0,0,0,0.5)] transform transition-transform duration-500 ease-in-out flex flex-col overflow-hidden ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {/* 1. USER HEADER SECTION */}
        <div className="bg-zoop-obsidian p-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 bg-zoop-obsidian rounded-2xl flex items-center justify-center text-xl shadow-[0_8px_24px_rgba(0,0,0,0.5)] shadow-zoop-moss/40">
              <User stroke="#b7e84b" />
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar py-6">
              ...
              {displayCategories.map((cat) => (
                <li key={cat}>
                  <Link
                    ...
                    onClick={onClose}
                  >
                    <span className="text-sm font-bold text-white group-hover:translate-x-2 transition-transform">
                      {cat}
                    </span>
                    <ChevronRight
                      width={16}
          ...
          <div className="h-px bg-white/5 mx-8 mb-10" />
          ...
          {/* THEME TOGGLE */}
          <section className="px-8 mb-10">
            <h4 className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em] mb-4">
              Appearance
            </h4>
            <button
              onClick={toggleTheme}
              className="w-full flex justify-between items-center group bg-white/5 border border-white/10 p-4 rounded-[16px] hover:bg-white/10 transition-colors"
            >
          ...
          <div className="mx-8 p-6 bg-zoop-canvas rounded-[2rem] border border-white/10">
